...
//创建X39SceneParseData对象
X39SceneParseData::X39SceneParseData()
{
}

void X39SceneParseData::parse(int taskId, int targetId, const QString &fileName, QMultiMap<QString, QVector<double>> &cpData, QMultiMap<QString, QVector<double>> &edData, QMultiMap<QString, QVector<double>> &noiselData
...
//检查数据文件序列化
bool X39SceneParseData::checkDatFileSerialization(int taskId, int targetId)
{
...
//数据文件反序列化
void X39SceneParseData::deserializeDatFile(int taskId, int targetId, const QString &datFileName, QMultiMap<QString, QVector<double> > &cpData, QMultiMap<QString, QVector<double> > &edData, QMultiMap<QString, QVector<double> > &noiselData, QMultiMap<QString, QVector<double> > &noisesData, QMultiMap<QString, QVector<double> > &fsdData, QMultiMap<QString, QVector<double> > &lofarData, QMultiMap<QString, QVector<double> > &demonData, QMultiMap<QString, QVector<quint8> > &echoData, QMultiMap<QString, QVector<quint8> > &noData)
{
    //创建cpFile等文件
    QFile cpFile(this->dataFileName(taskId, targetId, "CP"));
    QFile edFile(this->dataFileName(taskId, targetId, "ED"));
...
//格式时间
QString X39SceneParseData::formatTime(int year1, int year2, int month, int day, int hour, int minute, int seconds)
{
...
//文件名生成
QString X39SceneParseData::dataFileName(int taskId, int targetId, QString dType)
{
...
//解析后数据操作
void X39SceneParseData::operateDataAfterParse(int taskId, int targetId, QString dType, mwArray &inData, mwArray &inTime, QMultiMap<QString, QVector<double> > &output)
{
...
void X39SceneParseData::operateDataAfterParseAtByte(int taskId, int targetId, QString dType, mwArray &inData, mwArray &inTime, QMultiMap<QString, QVector<quint8> > &output)
{
    QString targetFileName = this->dataFileName(taskId, targetId, dType);
